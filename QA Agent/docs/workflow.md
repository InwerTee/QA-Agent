...
如果当前目标是推动团队试用和验证 self-test 流程,请先看 [QA Agent Pilot Workflow](pilot-workflow.md)。本文档偏技术 pipeline 说明,用于解释 agent 内部如何从 PRD / Excel 走到 normalized cases、triage、run 和 report。
...
v0.5 增加 `export-results`:它把某次 run 的工程化 `report.json` 转换回 Paragon 测试表视角。agent 不修改原 Excel,而是复制一份 workbook,在相关 sheet 原始内容最右侧追加 `Agent Result`,并填最终状态: `Passed`, `Partial`, `Failed`, `Blocked`, `Review`。v0.22 起,filled Excel 进一步追加 `Failure Category`、`Failure Summary`、`Recommended Action` 和 `Evidence`,让 pilot 结果可以直接给开发看。
...
v0.22 增加 Pilot Output & Failure Classification。agent 会把每条 case 的工程化结果转成开发可读输出:失败分类、问题摘要、建议动作、evidence 和 owner hint。网页展示分类汇总,Markdown 报告展示每条 case 的处理建议,filled Excel 在原表右侧追加用户可读结果列。这个阶段的重点是提升 pilot 反馈价值,不是追求所有 case 自动 pass。

v0.23 聚焦 pilot 输出可用性:网页分类增加中英双语说明,结果操作按钮前置并加亮,Excel Evidence 列不再大面积空白。没有浏览器截图的 case 会回指到 `report.md#case-id`;进入浏览器后失败的 R6 executor 会尽量保存 failure screenshot。
...
用户主要看 `<release>.agent-filled.xlsx`:它保持 Paragon 原表结构,并在每个相关测试用例行右侧填入 `Agent Result`、`Failure Category`、`Failure Summary`、`Recommended Action` 和 `Evidence`。
