...
当前仓库中的 `inputs/R6/` 是手工挑选出来的开发样例,用于验证 agent 的最小闭环。它不是未来同事使用 agent 时需要手动维护的输入格式。
...
当前 R6 MVP 跳过了第 1-3 步的自动解析,直接放了一份手工整理好的 `cases.normalized.json`。这是为了尽快验证执行和报告链路;后续需要补 `prepare` 阶段,让 agent 从真实 PRD / test case 文档生成这份中间输入。
...
```bash
npm run qa -- prepare --prd ./prd.pdf --cases ./test-cases.xlsx --release R6
